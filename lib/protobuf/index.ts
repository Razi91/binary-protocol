...
import pbschema from "protocol-buffers-schema";
...
module.exports = function createProtobufProtocol(proto, options) {
  ...
  function defaultFieldValue(field) {
    ...
    switch (field.type) {
      ...
      case "bytes":
        return _default !== undefined
          ? new Buffer(unquote(_default), "base64")
          : new Buffer(0);
      case "string":
      ...
      default:
        if (field._enum) {
          ...
          return _.get(
            definitions,
            [field.type, "values", _default, "value"],
            undefined
          );
        }
        return null;
      ...
      msg.name,
      {
        read: function(length) {
          ...
          _.each(definitions[fullName], function(field) {
            var v;
            if (field.map) {
              // remap back into js format
              self.context[field.name] = self.context[field.name].reduce(
                function(acc, e) {
                  acc[e.key] = e.val;
                  return acc;
                },
            ...
            if (
              self.context[field.name] === undefined &&
              field.required === true &&
              field.options.default === undefined
            ) {
              throw new Error(
                "Missing required field " + fullName + ":" + field.name
              );
            }
        ...
        write: function(value) {
          ...
          _.each(definitions[fullName], function(field) {
            ...
            if (field.map) {
              // remap into supported format
              _value = _.map(_value, function(val, key) {
  ...
  function parseProto(data) {
    ...
    _.each(definitions, function(fields, name) {
      ...
      _.each(fields, function(field) {
        ...
        if (field.type === "map") {
          // treat map as repeated map entry message with key/value fields
        ...
        if (!_.includes(primitives, field.type)) {
          paths = _.reduce(
            ("." + name).split("."),
            function(acc, val, ind, col) {
              acc.push(
                col
                  .slice(1, col.length - ind)
                  .concat(field.type)
                  .join(".")
              );
              return acc;
...
ProtobufProtocol.define("sfixed64", {
  read: function() {
    var l = new Long(
      ...
      this.buffer.readInt32LE(this.offset + 4)
    );
    this.offset += 8;
    return l;
  },
...
ProtobufProtocol.define("bytes", {
  ...
  write: function(value) {
    ...
    if (Buffer.isBuffer(value)) {
      this.UVarint(value.length).raw(value);
    } else {
...
ProtobufProtocol.define("_field", {
  read: function(msgName, fields) {
    ...
    if (field.repeated && field.options.packed !== "true") {
      // repeated, not packed
      fname.push(
        self.context[field.name] ? self.context[field.name].length : 0
      );
    }
    if (type === 2) {
...
ProtobufProtocol.define("_lengthLimited", {
  read: function(field) {
    ...
    if (field.type === "bytes" || field.type === "string") {
      ...
      // if (field._embedded) {
      _.get(self, field.type).call(
        self,
        undefined,
        self.offset + self.context.length
      );
